...
import { BookOpen, GraduationCap, Clock, Award, ChevronRight, PlayCircle, Star, Search, Bell } from "lucide-react"
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
        <div>
           <h1 className="text-5xl font-black tracking-tight text-zinc-900 mb-2">
             Welcome back, <span className="text-[#FF6600]">{session?.user?.name || "Learner"}</span>!
           </h1>
           <p className="text-zinc-500 font-medium">It's a great day to learn something new today.</p>
        </div>
        ...
        <div className="lg:col-span-2 space-y-12">
          ...
          <section>
            <div className="flex items-center justify-between mb-8">
              <h2 className="text-3xl font-black text-zinc-900">Continue Learning</h2>
            ...
            {continueLearning ? (
                  ...
                  <div className="flex-1 flex flex-col justify-between py-2">
                    <div>
                      <div className="flex items-center gap-2 mb-3">
                         <span className="px-3 py-1 bg-white rounded-lg text-[10px] font-black text-orange-600 border border-orange-100 shadow-sm uppercase tracking-widest">In Progress</span>
                         <span className="text-[10px] font-bold text-zinc-400">{continueLearning.course._count.sections} Modules</span>
                      </div>
                      ...
                      <p className="text-sm text-zinc-500 font-medium">By {continueLearning.course.instructor.name}</p>
                    </div>
                  ...
                  <div className="flex items-center">
                    <Button asChild className="bg-zinc-900 hover:bg-zinc-800 text-white rounded-[1.5rem] h-16 px-10 font-black shadow-xl transition-all group-hover:scale-105">
                      <Link href={`/learn/${continueLearning.course.slug}`}>
                        <PlayCircle className="w-6 h-6 mr-2" /> Resume
                      </Link>
                    </Button>
                  </div>
                </div>
              ...
              <div className="flex flex-col items-center justify-center py-24 bg-[#F8F9FA] rounded-[3rem] border-2 border-dashed border-zinc-200">
                ...
                <h3 className="text-2xl font-black text-zinc-900 mb-2">No courses started yet</h3>
                <p className="text-zinc-500 font-medium mb-10 max-w-sm text-center">Ready to jump into your first course? Explore our catalog to find your passion.</p>
          ...
          {/* Recent Courses */}
          <section>
             <h2 className="text-3xl font-black text-zinc-900 mb-8">Recent Courses</h2>
                ...
                {enrollments.slice(1, 3).map((enrollment) => (
                  ...
                  >
                    ...
                    <div className="flex-1">
                       ...
                       <p className="text-xs text-zinc-400 font-bold mb-6">By {enrollment.course.instructor.name}</p>
        ...
        <div className="space-y-10">
           ...
           <div className="bg-gradient-to-br from-[#1A1A1A] to-[#333] rounded-[3rem] p-10 text-white shadow-2xl relative overflow-hidden group">
              ...
              <div className="relative z-10">
                  ...
                  <h3 className="text-3xl font-black mb-3 leading-tight">Master <br/>New Skills</h3>
                  <p className="text-white/60 text-sm mb-10 leading-relaxed font-medium">Join our advanced learning tracks and get certified by top industry experts.</p>
              ...
              <div className="space-y-8">
                 {enrollments.slice(0, 4).map((e) => (
                    <div key={e.id} className="relative pl-8 border-l-2 border-zinc-100 pb-2 group">
                       ...
                       <p className="text-[10px] font-black text-zinc-400 mb-1 uppercase tracking-widest">{new Date(e.updatedAt).toLocaleDateString("en-US", { month: 'short', day: 'numeric' })}</p>
                       <p className="text-sm font-black text-zinc-900 group-hover:text-[#FF6600] transition-colors line-clamp-1">{e.course.title}</p>
                       <p className="text-xs text-zinc-500 mt-1 font-medium">Updated progress to {Math.round(e.progress)}%</p>
                    </div>
                 ))}
                 {enrollments.length === 0 && (
                    <div className="text-center py-10">
                       <p className="text-sm text-zinc-400 font-bold uppercase tracking-widest">No activity yet</p>
                    </div>
                 )}
              </div>
           </div>
