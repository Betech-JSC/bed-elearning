    ...
    <>
      {isCollapsed && (
        <button 
          ...
          className="absolute top-6 left-6 z-50 w-10 h-10 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl flex items-center justify-center text-zinc-600 shadow-sm hover:text-[#FF6600] hover:border-orange-200 dark:hover:border-orange-900 transition-all"
          title="Mở menu"
        >
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                 ...
                 <button 
                   ...
                   className="w-8 h-8 rounded-full hover:bg-zinc-100 dark:hover:bg-zinc-900 flex items-center justify-center text-zinc-400 hover:text-[#FF6600] transition-colors"
                   title="Ẩn menu"
                 >
        ...
        <div className="space-y-3">
          <div className="flex justify-between text-xs font-bold uppercase tracking-wider text-zinc-500">
            ...
            <div className="flex items-center gap-2">
               ...
               <span className="text-[#FF6600]">{progressPercentage}%</span>
            </div>
          </div>
          <div className="h-2.5 w-full bg-zinc-100 dark:bg-zinc-800 rounded-full overflow-hidden">
            <div 
              className="h-full bg-[#FF6600] transition-all duration-1000 ease-out shadow-[0_0_10px_rgba(255,102,0,0.3)]" 
      ...
      <ScrollArea className="flex-1">
        <Accordion defaultValue={sections.map(s => s.id)} className="w-full">
            ...
            <AccordionItem key={section.id} value={section.id} className="border-b last:border-b-0">
              <AccordionTrigger className="px-6 py-5 hover:no-underline hover:bg-zinc-50/80 dark:hover:bg-zinc-900/30 transition-all group">
                <div className="flex flex-col items-start gap-1">
                   <span className="text-[10px] text-[#FF6600] font-black uppercase tracking-widest">Phần {idx + 1}</span>
                   <span className="text-sm font-bold text-left group-hover:text-[#FF6600] transition-colors line-clamp-1">{section.title}</span>
                </div>
              </AccordionTrigger>
                    ...
                    return (
                      <Link
                        ...
                        className={cn(
                          ...
                          isActive 
                            ? "bg-white dark:bg-zinc-950 border-[#FF6600] text-[#FF6600] font-bold shadow-sm z-10" 
                      ...
                      >
                        <div className={cn(
                            ...
                            isActive ? "bg-[#FF6600] text-white shadow-lg shadow-orange-600/20" : "bg-zinc-100 dark:bg-zinc-800 text-zinc-400"
                        )}>
                            {lesson.isCompleted ? (
                        ...
                        {isActive && (
                           <div className="absolute right-3 w-1.5 h-1.5 rounded-full bg-[#FF6600] animate-pulse" />
                        )}
                      </Link>
                    )
                  })}
          ...
          <div className="p-6 bg-zinc-50 dark:bg-zinc-900/50 border-t mt-auto">
             <Link href="/dashboard" className="flex items-center justify-center gap-2 text-xs font-bold text-zinc-500 hover:text-[#FF6600] transition-colors py-2 group">
